...
Representation.prototype.toJSON = function() {
  ...
  if (embeddedKeys.length > 0) {
    ...
    embeddedKeys.forEach(embedKey => {
      if (self._embedded[embedKey] === null) {
        payload._embedded[embedKey] = null;
      } else {
...
Representation.prototype.embed = function(rel, self, entity) {
  ...
  let embedded = null;
  if (!this._embedded[qname] && entity === null) {
    this._embedded[qname] = null;
  } else {
    self = this._halacious.link(self, this._links.self.href);

    embedded = this.factory.create(entity, self, this._root);
